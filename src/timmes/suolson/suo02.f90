      subroutine suolson(t,x,trad_ev,tmat_ev,nstep,
     1     trad_bc_ev,opac,alpha)
      implicit none
      integer          nstep
      double precision t, trad_bc_ev, opac, alpha
      double precision x(nstep), trad_ev(nstep), tmat_ev(nstep)
cf2py intent(hide)     :: nstep
cf2py intent(out)      :: trad_ev, tmat_ev
cf2py integer          :: nstep
cf2py double precision :: t, trad_bc_ev, opac, alpha
cf2py double precision :: x(nstep), trad_ev(nstep), tmat_ev(nstep)
      integer i
      double precision zpos,erad,trad,tmat,trad_ev_out,tmat_ev_out
      do i=1,nstep
         zpos = x(i)
         call so_wave(t,zpos,trad_bc_ev,opac,alpha,
     1                erad,trad,trad_ev_out,tmat,tmat_ev_out)
         trad_ev(i) = trad_ev_out
         tmat_ev(i) = tmat_ev_out
      enddo
      end subroutine suolson
