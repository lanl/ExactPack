      subroutine riemann(t,npts,x,xi,
     1                          gamma,rhol,pl,ul,rhor,pr,ur,
     3                          rho,p,u)
      implicit none

cf2py intent(out)  :: rho, p, u
cf2py intent(hide) :: npts
cf2py integer :: npts
cf2py real    :: time, x(npts), xi
cf2py real    :: gamma, rhol, pl, ul, rhor, pr, ur
cf2py real    :: rho(npts), p(npts), u(npts)

      integer  npts
      real   x(npts), rho(npts), p(npts), u(npts)
      real   t, xi, gamma, rhol, pl, ul, rhor, pr, ur
...
c       write (6, 500)
c       write (6, 501) rho1, p1, u1
c       write (6, 502) 
c       write (6, 503) rho3, p3, u3
c       write (6, 504) rho4, p4, u4
c       write (6, 505) rho5, p5, u5
c
c       write (6, 506) xhd
c       write (6, 507) xft
c       write (6, 508) xcd
...
c..and do say what we found
c       write (6, 500)
c       write (6, 501) rho5, p5, u5
c       write (6, 602) rho4, p4, u4
c       write (6, 503) rho3, p3, u3
c       write (6, 604) 
c       write (6, 505) rho1, p1, u1
c
c       write (6, 609) xsh
c       write (6, 508) xcd
c       write (6, 507) xft
...
c$$$      open (unit=1, file = 'output',status='unknown')
c$$$      write (1, 1000)
c$$$1000  format (2x, 'i', 10x, 'x', 12x, 'density', 8x, 'pressure', 
c$$$     1             8x, 'velocity'/)
c$$$1001  format (i4, 4(2x, 1pe14.7))
c$$$      do i = 1, npts
c$$$       write (1,1001) i, x(i), rho(i), p(i), u(i)
c$$$      enddo
c$$$      close (1)
