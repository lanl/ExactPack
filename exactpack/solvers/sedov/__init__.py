r"""The Sedov blast wave problem models a rapid point-source explosion
of energy :math:`E_0` released at the origin :math:`r=0` at time :math:`t=0`.
...
:math:`P(r,0)=0` are zero everywhere in space (for :math:`r \ne 0`). At
...
element takes the form :math:`dV = 4\pi r^2 dr` for :math:`k=3`, :math:`dV =
2\pi r dr` for :math:`k=2`, and :math:`dV = dr` for :math:`k=1`. We take the gas
constant to have the value :math:`\gamma=7/5=1.4`, and for the spherically
symmetric case :math:`k=3` we also take :math:`E_0=0.851072\,{\rm erg}`, so that
...
class PlanarSedov(Sedov):
    ...
    parameters = {'geometry': Sedov.parameters['geometry'],
                  'eblast': Sedov.parameters['eblast'],
                  'gamma': Sedov.parameters['gamma']
    }
    geometry = 1
    eblast = 0.0673185
    gamma = 1.4
